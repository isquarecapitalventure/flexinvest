...
import { 
  TrendingUp, Users, Wallet, Banknote, MessageSquare, Home, LogOut, 
  Menu, X, Eye, CheckCircle, XCircle, Loader2, Clock, ShieldCheck
} from 'lucide-react';
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2">
            ...
            <span className="font-heading text-2xl font-bold text-white">FlexInvest Admin</span>
          </div>
        </div>
        ...
        <Card className="bg-slate-900/60 backdrop-blur-md border-slate-800">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full btn-primary" disabled={loading} data-testid="admin-login-btn">
        ...
        <div className="flex flex-col h-full">
          <div className="p-6 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-heading text-xl font-bold text-white">Admin</span>
            </div>
  ...
  return (
    <AdminLayout>
      <div className="space-y-8" data-testid="admin-dashboard">
        <div>
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-white mb-2">Admin Dashboard</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card className="bg-slate-900/60 backdrop-blur-md border-slate-800">
              <CardContent className="p-6">
                ...
                <div className="font-mono text-xs text-slate-500 uppercase tracking-wider mb-1">Total Users</div>
                <div className="font-heading text-2xl font-bold text-white">{stats.total_users}</div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {stats.pending_deposits > 0 && (
                    <span className="badge-pending">{stats.pending_deposits} pending</span>
                  )}
                </div>
                <div className="font-mono text-xs text-slate-500 uppercase tracking-wider mb-1">Total Deposited</div>
                <div className="font-heading text-2xl font-bold text-emerald-400">{formatCurrency(stats.total_deposited)}</div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {stats.pending_withdrawals > 0 && (
                    <span className="badge-pending">{stats.pending_withdrawals} pending</span>
                  )}
                </div>
                <div className="font-mono text-xs text-slate-500 uppercase tracking-wider mb-1">Total Withdrawn</div>
                <div className="font-heading text-2xl font-bold text-red-400">{formatCurrency(stats.total_withdrawn)}</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-slate-900/60 backdrop-blur-md border-slate-800">
              <CardContent className="p-6">
                ...
                <div className="font-mono text-xs text-slate-500 uppercase tracking-wider mb-1">Active Investments</div>
                <div className="font-heading text-2xl font-bold text-white">{stats.active_investments}</div>
              </CardContent>
            </Card>
          </div>
        )}
        ...
        {stats && (
          <div className="grid md:grid-cols-2 gap-6">
            <Card className="bg-slate-900/60 backdrop-blur-md border-slate-800">
              <CardHeader>
                <CardTitle className="font-heading text-lg text-white">Pending Actions</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                  ...
                  <span className="font-bold text-yellow-400">{stats.pending_deposits}</span>
                </div>
                <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                  <div className="flex items-center gap-3">
                    <Clock className="w-5 h-5 text-yellow-400" />
                  ...
                  <span className="font-bold text-yellow-400">{stats.pending_withdrawals}</span>
                </div>
                <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                  <div className="flex items-center gap-3">
                  ...
                  <span className="font-bold text-yellow-400">{stats.open_complaints}</span>
                </div>
              </CardContent>
            </Card>
          </div>
...
export const AdminUsersPage = () => {
  ...
  const handleCreditWallet = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.post(`${API_URL}/admin/credit-wallet`, {
  ...
  return (
    <AdminLayout>
      <div className="space-y-8" data-testid="admin-users-page">
        <div>
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-white mb-2">Users</h1>
              ...
              <table className="w-full">
                <thead className="bg-slate-900">
                  <tr className="border-b border-slate-800">
                    ...
                    <th className="text-left py-4 px-6 text-slate-400 font-medium text-sm">Wallet</th>
                  ...
                  {users.map((user) => (
                    <tr key={user.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                      <td className="py-4 px-6">
                        ...
                        <div className="text-slate-400 text-sm">{user.phone}</div>
                      </td>
        ...
        {/* Credit Wallet Modal */}
        <Dialog open={creditModal.open} onOpenChange={(open) => setCreditModal({ open, user: creditModal.user })}>
          <DialogContent className="bg-slate-900 border-slate-800 text-white">
            <DialogHeader>
              <DialogTitle className="font-heading text-xl">Credit Wallet</DialogTitle>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setCreditModal({ open: false, user: null })} className="btn-secondary">
                Cancel
              </Button>
              <Button onClick={handleCreditWallet} className="btn-primary" disabled={submitting}>
...
export const AdminDepositsPage = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [proofModal, setProofModal] = useState({ open: false, deposit: null, proof: null });
  const [actionModal, setActionModal] = useState({ open: false, deposit: null, action: null });
  ...
  const viewProof = async (deposit) => {
    try {
      const response = await axios.get(`${API_URL}/admin/deposits/${deposit.id}/proof`, {
        headers: { Authorization: `Bearer ${adminToken}` }
      });
  ...
  const handleAction = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.put(`${API_URL}/admin/deposits/${actionModal.deposit.id}`, {
  ...
  return (
    <AdminLayout>
      <div className="space-y-8" data-testid="admin-deposits-page">
        <div>
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-white mb-2">Deposits</h1>
                  ...
                  {deposits.map((deposit) => (
                    <tr key={deposit.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                      <td className="py-4 px-6">
                        ...
                        <div className="text-slate-400 text-sm">{deposit.user_email}</div>
                      </td>
                          ...
                          {deposit.status === 'pending' && (
                            <>
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => setActionModal({ open: true, deposit, action: 'approved' })}
                              ...
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => setActionModal({ open: true, deposit, action: 'rejected' })}
        ...
        {/* Proof Modal */}
        <Dialog open={proofModal.open} onOpenChange={(open) => setProofModal({ ...proofModal, open })}>
          <DialogContent className="bg-slate-900 border-slate-800 text-white max-w-2xl">
            <DialogHeader>
              <DialogTitle className="font-heading text-xl">Payment Proof</DialogTitle>
              <DialogDescription className="text-slate-400">
                {proofModal.deposit?.user_name} - {formatCurrency(proofModal.deposit?.amount || 0)}
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
              {proofModal.proof && (
          ...
          <DialogContent className="bg-slate-900 border-slate-800 text-white">
            <DialogHeader>
              <DialogTitle className="font-heading text-xl">
                {actionModal.action === 'approved' ? 'Approve' : 'Reject'} Deposit
              </DialogTitle>
              <DialogDescription className="text-slate-400">
                {formatCurrency(actionModal.deposit?.amount || 0)} from {actionModal.deposit?.user_name}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              ...
              {actionModal.action === 'approved' && (
                <p className="text-slate-300">This will credit the user's wallet with {formatCurrency(actionModal.deposit?.amount || 0)}.</p>
              )}
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setActionModal({ open: false, deposit: null, action: null })} className="btn-secondary">
                Cancel
              </Button>
              <Button 
