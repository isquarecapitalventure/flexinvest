...
import { Banknote, Building, CreditCard, User, Loader2, AlertCircle, CheckCircle } from 'lucide-react';
...
const NIGERIAN_BANKS = [
  ...
  'Heritage Bank', 'Jaiz Bank', 'Keystone Bank', 'Kuda Bank', 'OPay', 'Palmpay',
  'Polaris Bank', 'Providus Bank', 'Stanbic IBTC Bank', 'Standard Chartered Bank',
  'Sterling Bank', 'SunTrust Bank', 'Titan Trust Bank', 'Union Bank', 'United Bank for Africa (UBA)',
...
export const WithdrawPage = () => {
    ...
    const fetchData = async () => {
      try {
        const [bankRes, withdrawalsRes] = await Promise.all([
          axios.get(`${API_URL}/user/bank-account`, { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const handleWithdraw = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.post(`${API_URL}/withdrawals/create`, { amount }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      // Refresh withdrawals
      const withdrawalsRes = await axios.get(`${API_URL}/withdrawals/history`, { headers: { Authorization: `Bearer ${token}` } });
      setWithdrawals(withdrawalsRes.data.withdrawals || []);
    } catch (error) {
    ...
    <DashboardLayout>
      <div className="space-y-8" data-testid="withdraw-page">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-heading text-2xl md:text-3xl font-bold text-white mb-2">Withdraw</h1>
            <p className="text-slate-400">Withdraw your earnings to your bank account</p>
          </div>
          <div className="text-right">
            <div className="text-sm text-slate-400">Available Balance</div>
            <div className="font-heading text-xl font-bold text-emerald-400" data-testid="available-balance">{formatCurrency(user?.wallet_balance || 0)}</div>
          </div>
        </div>
          ...
          <CardContent>
            {bankAccount ? (
              <div className="bg-slate-800/50 rounded-lg p-4 space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white font-medium">{bankAccount.bank_name}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white font-medium font-mono">{bankAccount.account_number}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white font-medium">{bankAccount.account_name}</div>
                  </div>
                </div>
              </div>
            ) : (
              <div className="text-center py-8">
                ...
                <Button onClick={() => setShowBankModal(true)} className="btn-primary" data-testid="add-bank-btn">
                  Add Bank Account
                </Button>
              </div>
            )}
        ...
        {/* Withdrawal History */}
        <div>
          <h2 className="font-heading text-xl font-semibold text-white mb-4">Withdrawal History</h2>
                  ...
                  <tbody>
                    {withdrawals.map((withdrawal) => (
                      <tr key={withdrawal.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        <td className="py-4 px-6 text-emerald-400 font-medium">{formatCurrency(withdrawal.amount)}</td>
                        ...
                        <td className="py-4 px-6 text-slate-400 text-sm">{withdrawal.admin_note || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <Dialog open={showBankModal} onOpenChange={setShowBankModal}>
          <DialogContent className="bg-slate-900 border-slate-800 text-white">
            <DialogHeader>
              <DialogTitle className="font-heading text-xl">{bankAccount ? 'Edit' : 'Add'} Bank Account</DialogTitle>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Select value={bankForm.bank_name} onValueChange={(value) => setBankForm({...bankForm, bank_name: value})}>
                  <SelectTrigger className="bg-slate-950/50 border-slate-800 text-white" data-testid="bank-name-select">
                    <SelectValue placeholder="Select your bank" />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-900 border-slate-800">
                    {NIGERIAN_BANKS.map((bank) => (
                      <SelectItem key={bank} value={bank} className="text-white hover:bg-slate-800">{bank}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowBankModal(false)} className="btn-secondary">
                Cancel
              </Button>
              <Button onClick={handleSaveBank} className="btn-primary" disabled={submitting} data-testid="save-bank-btn">
        ...
        <Dialog open={showWithdrawModal} onOpenChange={setShowWithdrawModal}>
          <DialogContent className="bg-slate-900 border-slate-800 text-white">
            <DialogHeader>
              <DialogTitle className="font-heading text-xl">Request Withdrawal</DialogTitle>
              <DialogDescription className="text-slate-400">
            ...
            <div className="space-y-4 py-4">
              <div className="bg-slate-800/50 rounded-lg p-4">
                ...
                <div className="font-heading text-2xl font-bold text-emerald-400">{formatCurrency(user?.wallet_balance || 0)}</div>
              </div>
              <div className="space-y-2">
              ...
              {bankAccount && (
                <div className="bg-slate-800/30 rounded-lg p-4 text-sm">
                  ...
                  <div className="text-slate-300 font-mono">{bankAccount.account_number}</div>
                  <div className="text-slate-300">{bankAccount.account_name}</div>
                </div>
              )}
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowWithdrawModal(false)} className="btn-secondary">
                Cancel
              </Button>
              <Button onClick={handleWithdraw} className="btn-primary" disabled={submitting} data-testid="confirm-withdraw-btn">
                {submitting ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
                Submit Request
              </Button>
            </DialogFooter>
